...
const Subscription = () => {
  return (
    <section
      id="subscription"
      className="bg-slate-50 min-h-screen py-24 sm:py-32 px-4 sm:px-8 md:px-12 flex flex-col items-center overflow-hidden w-full"
    >
      <div className="max-w-7xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <h3 className="text-4xl font-black italic text-brand-green mb-2">
                {plan.name}
              </h3>
              <p className="text-brand-green/70 font-bold text-xs uppercase tracking-widest mb-8">
                {plan.tagline}
              </p>

              <div className="mb-10 flex flex-col">
                <span className="text-5xl font-black text-brand-green tracking-tighter">
                  {plan.price}
                </span>
                <span className="text-brand-green/50 font-bold italic text-sm uppercase">
                  {plan.period}
                </span>
              </div>
              ...
              <ul className="space-y-5 mb-12 flex-1 w-full">
                {plan.features.map((feature, fIdx) => (
                  <li
                    key={fIdx}
                    className="flex items-center gap-3 text-brand-green/80 font-bold italic text-lg leading-tight"
                  >
              ...
              <button
                className={`w-full py-5 rounded-3xl font-black text-xl italic transition-all shadow-xl active:scale-95
                ${
                  plan.highlight
        ...
        <p className="mt-20 text-center text-brand-green font-bold italic opacity-60 max-w-2xl mx-auto">
          *All meal plans include fresh seasonal ingredients. Delivery times can
          be customized to your preference. Special dietary requirements?
          Contact our nutritionists.
        </p>
      </div>
    </section>
  );
